...
impl<C: openxr_data::Compositor> vr::IVRInput010_Interface for Input<C> {
    ...
    fn GetOriginLocalizedName(
        ...
        if let Some(hand) = index {
            ...
            if string_sections_to_include & 0x02 != 0 || string_sections_to_include & -1 != 0 {
                let controller_type = self.get_controller_string_tracked_property(hand, openvr::ETrackedDeviceProperty::ControllerType_String).unwrap_or(c"ControllerType");
    ...
    fn GetActionOrigins(
        ...
        origins[0] = self.left_hand_key.data().as_ffi();
        origins[1] = self.right_hand_key.data().as_ffi();
