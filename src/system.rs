...
impl vr::IVRSystem023_Interface for System {
    ...
    fn PollNextEventWithPoseAndOverlays(
        ...
        let handle = unsafe { overlay_handle.read() };
        if vr::IVROverlay027_Interface::PollNextOverlayEvent(overlay.as_ref(), handle, event, size)
        {
            let event_data = unsafe { event.as_ref().unwrap().data };
            unsafe { overlay_handle.write(event_data.overlay.overlayHandle) };
            return true;
        }
        false
    }
