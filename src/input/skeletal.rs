...
impl FingerState {
    ...
    fn lerp(&self, target: &Self, alpha: f32) -> Self {
        Self {
            index: self.index + (target.index - self.index) * alpha,
            middle: self.middle + (target.middle - self.middle) * alpha,
            ring: self.ring + (target.ring - self.ring) * alpha,
            pinky: self.pinky + (target.pinky - self.pinky) * alpha,
            thumb: self.thumb + (target.thumb - self.thumb) * alpha,
            time: target.time,
        }
    }
