...
pub struct ControllerData {
    ...
    pub skeleton_cache: Mutex<Box<Option<(xr::HandJointLocations, xr::HandJointVelocities)>>>,
...
impl TrackedDevice {
    ...
    pub fn get_hand_skeleton(
        ...
        if let Some(skeleton) = **skeleton_cache {
    ...
    pub fn clear_pose_cache(&self) {
        ...
        if let TrackedDeviceType::Controller(data) = self.get_type() {
            std::mem::take(&mut **data.skeleton_cache.lock().unwrap());
        }
    }
